...
const FormularioUsuarios = () => {
  ...
  const personaId = idUsuario ? parseInt(idUsuario) : 0;
  ...
  const [mostrarClave, setMostrarClave] = useState(false); // Estado para controlar la visibilidad de la contraseña
    ...
    const fetchPersona = async () => {
      if (personaId > 0) {
        try {
          ...
          setDomicilioObjeto(data.domicilio);
          setUserObjeto(data.user);
        } catch (error) {
      ...
      <Form>
        ...
        <button
          ...
          onClick={async (e) => {
            ...
            if (success) navigate("/grillaProducts");
          }}
        >
          Guardar
        </button>
